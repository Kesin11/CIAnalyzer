...
describe("CustomReportCollection", () => {
  ...
  it("set", async () => {
    expect(customReportCollection.customReports.get("custom")).toStrictEqual([
      customReport,
    ]);
  });
...
describe("aggregateCustomReportArtifacts", () => {
  it("aggregate CustomReportArtifact[]", () => {
    ...
    const actual = aggregateCustomReportArtifacts([
      customReportArtifact1,
      customReportArtifact2,
    ]);
