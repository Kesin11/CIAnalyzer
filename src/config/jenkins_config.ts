...
export const jenkinsYamlSchema = commonSchema.merge(
  z.object({
    baseUrl: z.string(),
    jobs: z
      .union([
...
const jenkinsConfigSchema = jenkinsYamlSchema
  .merge(
    z.object({
      jobs: z
        .object({
...
const migrateConfig = (
  ...
  if (config.correctAllJobs) {
    logger.warn(
      "`correctAllJobs` is deprecated. Please use collectAllJobs instead.",
    );
