...
export class BigqueryExporter implements Exporter {
  ...
  constructor(
    ...
    const workflowTable = config.reports?.find(
      (report) => report.name === "workflow",
    )?.table;
    const testReportTable = config.reports?.find(
      (report) => report.name === "test_report",
    )?.table;
    if (!workflowTable || !testReportTable) {
      throw new Error(
        "Must need both 'workflow' and 'test_report' table name in exporter.bigquery.reports config.",
      );
    }
    this.table = {
    ...
    this.customReportTableInfo = new Map(
      config.customReports?.map((customReport) => {
        ...
        return [customReport.name, { table: customReport.table, schemaPath }];
      }),
    );
  ...
  private async export(
    ...
    // Load to BigQuery
    this.logger.info(
      `Loading ${tmpJsonPath} to ${this.dataset}.${table}. tmp file will be deleted if load complete with no error.`,
    );
    try {
      await this.bigquery
      ...
      this.logger.error(
        `ERROR!! loading ${tmpJsonPath} to ${this.dataset}.${table}`,
      );
      throw error;
    }
  ...
  async exportTestReports(reports: TestReport[]) {
    await this.export(
      reports,
      this.table.testReport,
      BQ_SCHEMA_PATHS.test_report,
    );
  }
  ...
  async exportCustomReports(customReportCollection: CustomReportCollection) {
    for (const [
      reportName,
      customReports,
    ] of customReportCollection.customReports) {
      const table = this.customReportTableInfo.get(reportName);
      if (table === undefined)
        throw new Error(
          `name: '${reportName}' does not exists in exporter.bigquery.customReports config!!`,
        );
