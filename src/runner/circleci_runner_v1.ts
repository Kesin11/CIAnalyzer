...
export class CircleciRunnerV1 implements Runner {
  ...
  constructor(
    ...
    this.client = new CircleciClient(
      CIRCLECI_TOKEN,
      this.logger,
      options,
      this.config?.baseUrl,
  ...
  async run(): Promise<Result<unknown, Error>> {
    ...
    for (const repo of this.config.repos) {
      ...
      try {
        ...
        const workflowRuns = await this.client.fetchWorkflowRuns(
          repo.owner,
          repo.repo,
          repo.vcsType,
          lastRunId,
        );
        const tagMap = await this.githubClient.fetchRepositoryTagMap(
          repo.owner,
          repo.repo,
        );
        ...
        for (const workflowRun of workflowRuns) {
          ...
          const customReportArtifactsList = await Promise.all(
            workflowRun.build_nums.map((buildNum) => {
  ...
  migrateLastRun(repoFullname: string) {
    ...
    if (metadata === undefined || metadata.version <= META_VERSION) {
      ...
      throw new Error(
        `${repoFullname} was executed with ${metadata.version} that is newer than ${CircleciRunnerV1.name}`,
      );
    }
  }
}
