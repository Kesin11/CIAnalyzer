...
export class JenkinsRunner implements Runner {
  ...
  async run(): Promise<Result<unknown, Error>> {
    ...
    if (!this.client)
      return failure(new Error("this.client must not be undefined"));
    if (!this.analyzer)
      return failure(new Error("this.analyzer must not be undefined"));
    ...
    for (const job of jobs) {
      ...
      try {
        ...
        if (runs.length < 1) {
          this.logger.warn(
            `SKIP ${job.name}: it does not have any runs data for some reason`,
          );
          continue;
        }
        ...
        for (const run of runs) {
          ...
          // Create report
          const report = this.analyzer.createWorkflowReport(
            job.name,
            run,
            build,
          );
  ...
  private async getJobs(): Promise<JenkinsConfig["jobs"]> {
    ...
    const allJobs = await this.client.fetchJobs(
      this.config.collectAllJobs?.isRecursively ?? false,
    );
    const allJobMap = new Map(allJobs.map((job) => [job.name, job]));
    const configJobs = this.config.jobs.filter((job) =>
      allJobMap.get(job.name),
    );
    ...
    if (this.config.collectAllJobs) {
      const notConfigJobs = allJobs.filter((job) => {
        return (
          configJobs.find((configJob) => configJob.name === job.name) ===
          undefined
        );
      });
      ...
      for (const { jobName, resultPromise } of buildRespones) {
        ...
        if (
          result.isSuccess() &&
          result.value &&
          result.value.timestamp >= thresholdTimestamp
        ) {
          otherJobs.push({
            name: jobName,
            testGlob: [],
