...
import { dropWhile, maxBy, sumBy, takeWhile } from "../collection.js";
...
export class BitriseAnalyzer implements Analyzer {
  ...
  createWorkflowReport(
    app: App,
    build: BuildResponse,
    buildLog: BuildLogResponse | null,
  ): WorkflowReport {
    ...
    const startedAt = new Date(
      build.started_on_worker_at ?? build.triggered_at,
    );
  ...
  parseBuildLog(BuildLogResponse: BuildLogResponse): StepLog[] {
    ...
    const chunks = dropWhile(
      BuildLogResponse.log_chunks,
      (chunk) => !chunk.chunk.includes("bitrise summary"),
    );
    if (!chunks) return [];
