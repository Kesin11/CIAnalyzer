...
import type {
  ...
  BuildResponse,
  CauseAction,
  GhprbParametersAction,
  BuildData,
  ParametersAction,
  TimeInQueueAction,
} from "../client/jenkins_client.js";
import { sumBy, first } from "../collection.js";
...
export class JenkinsAnalyzer implements Analyzer {
  ...
  detectRepository(build: BuildResponse): string {
    ...
    switch (action._class) {
      ...
      case "org.jenkinsci.plugins.ghprb.GhprbParametersAction": {
        const repoParam = action.parameters.find(
          (param) => param.name === "ghprbAuthorRepoGitUrl",
        );
  ...
  detectHeadSha(build: BuildResponse): string {
    ...
    switch (action._class) {
      ...
      case "org.jenkinsci.plugins.ghprb.GhprbParametersAction": {
        const repoParam = action.parameters.find(
          (param) => param.name === "ghprbActualCommit",
        );
        return repoParam?.value ?? "";
      }
    }
