...
import type {
  ...
  CircleciStatus,
  TestResponse,
} from "../client/circleci_client.js";
...
export class CircleciAnalyzer implements Analyzer {
  ...
  createWorkflowReport(
    ...
    return {
      ...
      parameters: [],
      queuedDurationSec: diffSec(
        createdAt,
        min(jobs.map((job) => new Date(job.start_time)))!,
      ),
      commitMessage: "",
      actor: "",
      url: "",
      ...
      .map((test) => {
        const testCases: TestCase[] = test.tests.map((test) => {
          return {
            ...
            time: test.run_time,
            failure:
              test.result === "failure" ? [{ inner: test.message }] : undefined,
            skipped:
              test.result === "skipped"
