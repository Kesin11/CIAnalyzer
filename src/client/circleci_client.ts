...
export class CircleciClient {
  ...
  constructor(
    ...
    if (baseUrl && path.basename(baseUrl) !== "v1.1") {
      throw new Error(
        `${CircleciClient.name} accepts only "/api/v1.1/" But your baseUrl is ${baseUrl}`,
      );
    }
    this.#options = options;
    const httpLogger = logger.getSubLogger({ name: CircleciClient.name });
  ...
  async fetchWorkflowRuns(
    ...
    const workflowRuns: WorkflowRun[] = Object.values(groupedBuilds).map(
      (builds) => {
  ...
  filterWorkflowRuns(rawRuns: WorkflowRun[]): WorkflowRun[] {
    ...
    let runs = rawRuns.filter((run) => {
      return !run.lifecycles.some((lifecycle) => lifecycle === "not_run");
    });

    const inprogressRuns = runs.filter((run) => {
      return !run.lifecycles.every((lifecycle) => lifecycle === "finished");
    });
    const firstInprogress = minBy(inprogressRuns, (run) => run.last_build_num);
    runs = firstInprogress
      ? runs.filter(
          (run) => run.last_build_num < firstInprogress.last_build_num,
        )
      : runs;
    return runs;
  }
  ...
  async fetchWorkflowJobs(workflowRun: WorkflowRun) {
    return await Promise.all(
      workflowRun.build_nums.map((buildNum) => {
  ...
  async fetchArtifacts(
    ...
    const pathResponses = artifactsResponse.map((artifact) => {
      const response = this.#http.get(artifact.url, {
  ...
  async fetchWorkflowCustomReports(
  ...
  async fetchCustomReports(
    ...
    const artifactsResponse = await this.fetchArtifactsList(
      owner,
      repo,
      vcsType,
      runId,
