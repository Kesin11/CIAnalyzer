...
export class CircleciClientV2 {
      ...
      await this.#axios.get("v2/me", {
        validateStatus: (status) => {
          return (
            ...
            status === 401
          ); // It means endpoint of API v2 is exists.
        },
      });
    } catch (error) {
      return failure(
        new Error(`${this.#baseUrl} unavailable API v2`, {
  ...
  async fetchWorkflowRuns(
    ...
    for (const recentPipeline of recentPipelines) {
      const workflows =
        await this.fetchV2Api<ListWorkflowsByPipelineIdResponse>(
          `v2/pipeline/${recentPipeline.id}/workflow`,
        );
  ...
  private filterPipelines(rawPipelines: Pipeline[]): Pipeline[] {
    ...
    const inprogressPipeline = pipelines.filter((pipeline) => {
      return pipeline.workflows.every(
  ...
  private async fetchWorkflowJobs(workflowId: string) {
    const jobs = await this.fetchV2Api<ListWorkflowJobsResponse>(
  ...
  private async fetchArtifactsList(
    jobNumber: number,
    projectSlug: string,
