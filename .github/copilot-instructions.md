...
- Configuration validation is defined with Zod under `src/config/`, and `schema.json` is the JSON Schema for the `ci_analyzer.yaml` config file. When adding or changing config options, update the relevant Zod schema, `schema.json`, tests, and user-facing samples or docs together.
